...
// 깨지면 대원 앱 전체(사후보고서 저장 포함)의 저장/제출 피드백이 조용히 무너진다.
...
describe('apiRequest', () => {
  ...
  it('query 파라미터 중 undefined·빈문자열은 URL에서 빠진다', async () => {
    ...
    await apiRequest('/api/v1/reports', { query: { status: 'DRAFT', team: undefined, keyword: '' } })

    const [calledUrl] = vi.mocked(fetch).mock.calls[0]
    expect(calledUrl).toBe('/api/v1/reports?status=DRAFT')
  })

  it('204 응답은 본문을 파싱하지 않고 undefined를 반환한다', async () => {
    vi.mocked(fetch).mockResolvedValue(new Response(null, { status: 204 }))

    await expect(apiRequest('/api/v1/reports/1')).resolves.toBeUndefined()
  })
  ...
  it('실패 응답은 서버가 준 message를 담은 ApiError를 던진다', async () => {
    ...
    const error = await apiRequest('/api/v1/reports').catch((e: unknown) => e)
  ...
  it('본문 없는 실패 응답은 HTTP 상태코드를 담은 기본 메시지로 ApiError를 던진다', async () => {
    ...
    await expect(apiRequest('/api/v1/reports')).rejects.toThrow('요청이 실패했습니다 (HTTP 500)')
  })
})
